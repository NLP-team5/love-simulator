...
if (
    ...
    logger.warning(
        "🚨 SECURITY WARNING: Using default SECRET_KEY in production! Set SECRET_KEY environment variable."
    )
...
class Scenario(db.Model):
    ...
    scenes = db.relationship(
        "Scene", backref="scenario", lazy="dynamic", cascade="all, delete-orphan"
    )
...
class Scene(db.Model):
    ...
    scenario_name = db.Column(
        db.String(80), db.ForeignKey("scenario.name"), nullable=False, index=True
    )
    choices = db.relationship(
        "Choice", backref="scene", lazy="select", cascade="all, delete-orphan"
    )
...
def get_scene(scenario_name, scene_id):
    ...
    """
    scene_data = (
        Scene.query.filter_by(scenario_name=scenario_name, sceneId=scene_id)
        .options(db.joinedload(Scene.choices))
        .first()
    )
    if not scene_data:
...
def get_rankings():
    ...
    """
    rankings = (
        Ranking.query.order_by(Ranking.score.desc(), Ranking.timestamp.asc())
        .limit(100)
        .all()
    )
    return jsonify(
...
def add_ranking():
    ...
    try:
        ...
        if not data or not all(
            k in data for k in ["nickname", "score", "scenario_title"]
        ):
            abort(
                400,
                description="필수 데이터(nickname, score, scenario_title)가 누락되었습니다.",
            )
        ...
        if not re.match(r"^[가-힣a-zA-Z0-9_\s]+$", nickname):
            abort(
                400,
                description="닉네임에는 한글, 영문, 숫자, 밑줄, 공백만 사용 가능합니다.",
            )
        ...
        rank_count = (
            Ranking.query.filter(
                Ranking.score > new_ranking.score,
                Ranking.scenario_title == data["scenario_title"],
            ).count()
            + 1
        )
...
def not_found_error(e):
    ...
    return (
        jsonify(error="Not Found", message="요청한 API 리소스를 찾을 수 없습니다."),
        404,
    )
...
def handle_error(e):
    ...
    return (
        jsonify(
            error="Internal Server Error", message="서버 내부 오류가 발생했습니다."
        ),
        500,
    )
